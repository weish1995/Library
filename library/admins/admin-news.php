...
<body>
	<?php 
		...
		if (isset($_GET['deleteId'])) {
			if ($db->singleOp('delete from news where newId = "'.$_GET['deleteId'].'" and newsType = "新闻通知"')) {
		...
		// 获取所有记录--用于分页
		// sql语句
		$logSql = 'select * from news join admin on news.adminId = admin.adminId '.$searchStr.' and newsType = "新闻通知"';
	...
	<div class="content">
		...
		<div class="content-header">
			...
			<small class="content-subtitle">图书馆的实时通知，新闻等</small>
			<div class="content-breadcrumb">
				<span class="content-breadcrumb-span">
				<i class="content-breadcrumb-icon header-menu-icon-link"></i>资源信息
				</span>>
				<span class="content-breadcrumb-span">新闻通知</span>
			</div>
		</div>

		<div class="wrap wrap-users">
			<h3 class="wrap-title">
				<i class="wrap-title-icon header-menu-icon-email"></i>NEWS
			</h3>

			<form class="wrap-search" action="" method="get">
				<a class="wrap-add" href="admin-news-edit.php"></a>
			...
			<div class="mytable">
				...
				<div class="mytable-tr">
					...
					<div class="mytable-tr-td">
						<a class="icon-op delete" title="删除" href="admin-news.php?deleteId=<?php echo $infos[$i]['newId'];?>">删除</a>
						<a class="icon-op edit" title="编辑" href="admin-news-edit.php?id=<?php echo $infos[$i]['newId'];?>">编辑</a>
					</div>
				</div>
				<?php
					}
			...
			<div class="pages">
				...
				<?php 
					if ($page == 1) {
						...
						echo '<a class="tobutton pages-op" href="admin-news.php?page=1'.$sortStr.'">首页</a>'
							.'<a class="tobutton pages-op" href="admin-news.php?page='.($page-1).$sortStr.'">上一页</a>';
					}
					...
					if ($page == $allPages) {
						...
						echo '<a class="tobutton pages-op" href="admin-news.php?page='.($page+1).$sortStr.'">下一页</a>'
							.'<a class="tobutton pages-op" href="admin-news.php?page='.$allPages.$sortStr.'">尾页</a>';
					}
				?>
			</div>
		</div>
